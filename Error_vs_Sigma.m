...
%                [Theta(Round+1,1),x_partial,y_partial] = Bimodal_Drifting(Theta(Round,1),2*Sigma,Delta_t, Sigma); %Bimodal Drifting
        ...
        d = sprintf('Sigma %f, Error %f',Sigma,Error_Sig);
        disp(d);
        ...
        CI_L1 = mean(Path_Total)-1.96*sqrt(var(Path_Total)/num);
        CI_L2 = mean(Path_Total)+1.96*sqrt(var(Path_Total)/num);
        Trial_Num = Write_To_Excel_4(Trial_Num,'Persist',Radius,Veloc,Delta_t,Sigma,num,Time_Pen,Error_Sig,mean(Time_Total),mean(Path_Total),CI_T1,CI_T2,CI_L1,CI_L2,mean(time));
