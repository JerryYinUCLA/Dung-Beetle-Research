...
for i = 1:num
    while (Distance(Position(Round,1,i), Position(Round,2,i)) < Radius)
        ...
        p1 = NORM_CDF(Theta(Round,1), Delta_t, Sigma, 1);
        p2 = NORM_CDF(Theta(Round,1), Delta_t, Sigma, 2);
        p3 = NORM_CDF(Theta(Round,1), Delta_t, Sigma, 3);
        p4 = NORM_CDF(Theta(Round,1), Delta_t, Sigma, 4);
        Random_Num = rand();
        if rand()<p1
            Theta(Round+1,1) = 0;
            x_partial = cos(0);
            y_partial = sin(0);
        elseif rand()<p1+p2
            Theta(Round+1,1) = pi/2;
            x_partial = cos(pi/2);
            y_partial = sin(pi/2);
        elseif rand()<p1+p2+p3
            Theta(Round+1,1) = pi;
            x_partial = cos(pi);
            y_partial = sin(pi);
        else
