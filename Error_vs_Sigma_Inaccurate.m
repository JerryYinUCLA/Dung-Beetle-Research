...
Trial_Num = Write_To_Excel_5(Trial_Num,'Persist',Radius,Veloc,Delta_t,Sigma,num,Time_Pen,0,Time_T,Path_T,0,CI_T1,CI_T2,CI_L1,CI_L2,0);

while (abs(Switch1)<0.1)
    Error_Sig = pi()/60; % Sigma of Adjustment
    min_time=9999;
    Switch2 = 0;
    Time_Pen = 5;
    num = 500;
...
%                [Theta(Round+1,1),x_partial,y_partial] = Bimodal_Drifting(Theta(Round,1),2*Sigma,Delta_t, Sigma); %Bimodal Drifting
               ...
               if (abs(flag-1)<0.1)
                   [Theta(Round,1),x_partial,y_partial]=Adjust_Dir_Err(Position(Round,1,i),Position(Round,2,i),Sigma_ADJ);
        ...
        if (abs(Error_Sig-pi()+pi()/60)<0.05)
            Switch2 = 1;
        end
        Error_Sig = Error_Sig + pi()/60;
    end
    Trial_Num = Write_To_Excel_5(Trial_Num,'Persist',Radius,Veloc,Delta_t,Sigma,num,Time_Pen,Sigma_ADJ,min_Time_Total,min_Path_Total,Opt_Error_Sig,CI_T1,CI_T2,CI_L1,CI_L2,mean_check);
    Sigma_ADJ = Sigma_ADJ + pi()/30;    
    if (abs(Sigma_ADJ-pi()-pi()/30)<0.01)
        Switch1 = 1;
    end
end
