...
Delta_t = 0.1;
Theta = zeros(1000,1);
Theta(1,1) = pi(); % First assume all dung beeles go towards pi
...
for i = 1:num
    while (Distance(Position(Round,1,i), Position(Round,2,i)) < Radius)
        ...
        [Sigma,indicator] = Discrete_Sigma(C_T,R,Radius);
        Time(i,indicator) = Time(i,indicator) + Delta_t;
...
disp([mean(Time(:,1)) mean(Time(:,2)) mean(Time(:,3)) mean(Time(:,4))...
    mean(Time(:,5)) mean(Time(:,6))]);